...
- `<MacOSBundleShortVersion>` — bundle version string, consisting of exactly three period-separated non-negative integers, shown to the user. Maps to the [`CFBundleShortVersionString`](https://developer.apple.com/documentation/bundleresources/information-property-list/cfbundleshortversionstring) key in the `Info.plist` file. Defaults to the value of `<MacOSBundleVersion>`, padded with `.0` components as needed.
